...
class LogsDataLoader:
    ...
    def _load_data_from_sheet(self, sheet_id, worksheet_id, date_column, numeric_columns):
        """
...
class StocksValuationDataLoader:
    def __init__(self, logs_folder):
        """
        Initializes the StocksValuationDataLoader instance with the given logs folder path.
    ...
    def _load_data_from_sheet(self,
        ...
        try:
            ...
            # Fetch data from the Google Sheet
            df = self.reader.get_sheet_as_dataframe(sheet_id, worksheet_id, drop_last_rows, n_rows)
    ...
    def load_asset_stocks_data(self):
        ...
        Returns:
            pd.DataFrame: Preprocessed stocks data.
        """
        sheet_id = os.environ.get('stocks_valuation_sheet_id')
        worksheet_id = os.environ.get('stocks_valuation_wallet_worksheet_id')

        # Define numeric columns
        numeric_columns = ['QUANTIDADE', 'PREÇO ATUAL',
                           'PREÇO TETO BAZIN', 'PREÇO TETO GRAHAM', 'PREÇO TETO GORDON',
                           'ÍNDICE PEG', 'TOTAL INVESTIDO', '% NA CARTEIRA', 'DY', 'DPA',
                           'DIVIDENDO ANUAL','MÉDIA DE EXPECTATIVA DE CRESCIMENTO',
                           'VALOR FINAL']

        return self._load_data_from_sheet(sheet_id, worksheet_id, numeric_columns, True, 2)
